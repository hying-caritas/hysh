...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (defun convert-to-run-form (form)
    (let ((car-form (car form)))
      (cond
	((typep car-form 'string) (cons 'run form))
	((and (listp car-form) (eql 2 (length car-form))
	      (eq (car car-form) 'quote))
	 (list* 'run (cadr car-form) (cdr form)))
	(t form))))
  (defun process-task-form (form-in)
